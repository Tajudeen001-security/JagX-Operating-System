/* Compact SHA-256 — real implementation for secure boot hashing */
#include "sha256.h"
...
void sha256(const uint8_t* data, size_t len, uint8_t out[32]) {
    ...
    size_t i = 0;
    while (i + 64 <= len) {
        for (int j = 0; j < 64; j++) block[j] = data[i+j];
        sha256_transform(state, block);
        i += 64;
    }
    size_t rem = len - i;
    for (size_t j = 0; j < rem; j++) block[j] = data[i+j];
    block[rem] = 0x80;
    if (rem < 56) {
    ...
    for (int j = 0; j < 8; j++) block[63-j] = (uint8_t)(bitlen >> (j*8));
    sha256_transform(state, block);
    for (int j = 0; j < 8; j++) {
