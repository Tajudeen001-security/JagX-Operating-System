/* TLS 1.2-style PRF using HMAC-SHA256 building blocks */
...
void tls_prf_sha256(const uint8_t* secret, size_t secret_len,
    ...
    /* P_hash = HMAC(secret, A(i) + seed) expansions */
    uint8_t a[32];
    uint8_t buf[64 + 32 + 128];
    size_t label_len = 0;
    while (label[label_len]) label_len++;

    /* A(0) = seed = label + seed_param */
    uint8_t seed_full[64];
    size_t sl = 0;
    for (size_t i = 0; i < label_len && sl < 64; i++) seed_full[sl++] = (uint8_t)label[i];
    for (size_t i = 0; i < seed_len && sl < 64; i++) seed_full[sl++] = seed[i];
    ...
    while (produced < out_len) {
        ...
        for (int i = 0; i < 32 && produced < out_len; i++)
